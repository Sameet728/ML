...
class WalkForwardEngine:
    ...
    def run(
        ...
        for fold_idx, (tr_start, tr_end, te_start, te_end) in enumerate(folds):
            ...
            # ── Check for resumable fold checkpoint ──
            fold_ckpt_path = Path(cfg.paths["reports"]) / f"fold_{fold_idx}_checkpoint.pkl"
            if fold_ckpt_path.exists():
                log.info(f"Fold {fold_idx+1}: Loading from checkpoint {fold_ckpt_path.name}...")
                try:
                    fold = joblib.load(fold_ckpt_path)
                    self.fold_results.append(fold)
                    all_probs.append(fold.predictions)
                    all_quality.append(fold.quality_scores)
                    all_labels.append(fold.labels)
                    all_regime.append(fold.regime_df)
                    continue
                except Exception as e:
                    log.warning(f"Failed to load checkpoint for fold {fold_idx+1}: {e}. Retraining...")
            ...
            # Save checkpoint
            try:
                joblib.dump(fold, fold_ckpt_path)
            except Exception as e:
                log.warning(f"Failed to save checkpoint for fold {fold_idx+1}: {e}")
    ...
    def _generate_folds(
        ...
        while True:
            ...
            # Wait 48 hours to ensure no label overlap (horizon is 24h) and add a small embargo
            te_start = tr_end + pd.Timedelta(hours=48)
